function getPHPResponseVar(responseText, varname, semicolon = true){
...
function login_php(username, password, callback){
    xmlHttpRequest("php/loginUser.php",
...
function signup_php(username, password, callback){
    xmlHttpRequest("php/checkUser.php", "username="+username,
        function(responseText){
            if (responseText.indexOf("username=Exists") != -1){
                ...
                xmlHttpRequest("php/createUser.php",
                    "username="+username+"&password="+password,
